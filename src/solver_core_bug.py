...
class QueenSolver:
    ...
    def _bisa_lanjut(self, row: int, col: int, posisi_queenn: List[Tuple[int, int]]) -> bool:
        # "Bisa lanjut kalau, ga di row dan column sama, dan kalau ga bersebelahan 8 kotak, namun BISA DIAGNOAL asal gak sebelahan"
        warna = self.papan_awal[row][col]
    ...
    def _backtrack(self, row: int, posisi_queen: List[Tuple[int, int]], progress_callback: Optional[Callable] = None) -> bool:
        if row == self.n: # Udah tempatin semua queen, keluar dari loop
            return True 
        ...
        for col in range(self.n):
            ...
            if self._bisa_lanjut(row, col, posisi_queen):
                ...
                if self._backtrack(row+1, posisi_queen, progress_callback):
                    return True # If (selanjutnya (nested loop, sampai akhir) aman) then (bisa keluar)
                
                # Kalau engga, maka ga jadi masukkin '#', harus ubah posisi
                posisi_queen.pop()
...
def validasi_warna_papan(papan: List[List[str]], n: int) -> Dict[str, any]:
    ...
    mungkin_ada_solusi = (jum_warna >= n) # n : jum queen
    warning = ""
    if jum_warna < n:
        ...
        warning = (f"Papan punya {jum_warna} warna untuk {n} queens. "
                  f"{jum_warna - n} wanra akan kosong. Kemungkinan aman, bisa ada solusi.")
        
    return {
        'jumlah_warna': jum_warna,
